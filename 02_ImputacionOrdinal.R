...
for (base in bases) {
  ...
  temp_df[colnames(temp_df) != "Username_Std"] <- lapply(temp_df[colnames(temp_df) != "Username_Std"], as.factor)
...
saveWorkbook(wb, file.path(dataPath, "Datos_ImputaciónOrd.xlsx"), overwrite = TRUE)

# Información relevante
assign("PrediccionesOrd", Predicciones, envir = .GlobalEnv)
...
save.image(file = "./data/02_ImputacionOrd.RData")
